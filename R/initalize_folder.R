...
#
# - ``$project_dir/image/`` for general plots
# - ``$project_dir/gviz/`` for Gviz plots
# - ``$project_dir/rds/`` for rds files
# - ``$project_dir/rds_methylation/`` for smoothed methylation
# - ``$project_dir/rds_cr/`` for CR-related rds files
# - ``$project_dir/temp/`` for temporary files
# - ``$project_dir/enriched_heatmap/`` for enriched heatmaps
# - ``$project_dir/txdb/`` for TxDb database
#
# == value
...
initialize_project_directory = function(project_dir) {
	...
	dir.create(paste0(project_dir, "/rds_methylation"), showWarnings = FALSE)
	...
	dir.create(paste0(project_dir, "/txdb"), showWarnings = FALSE)
	...
	message(qq("  @{project_dir}/rds_methylation/ for smoothed methylation"))
	...
	message(qq("  @{project_dir}/txdb/ for TxDb database"))
}
